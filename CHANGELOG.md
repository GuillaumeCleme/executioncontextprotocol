...
- **Runtime / CLI:** **`resolveMergedSystemConfig`** merges project **`ecp.config.*`** with **`~/.ecp/*`** so **global overrides local** on overlapping keys; **`--config <path>`** still loads one file only. **`resolveMergedSystemConfigRequired`** (used by **`ecp run`** / **`ecp validate`**) throws when **no** file is found so execution never proceeds without host policy on disk. **`assertHostPolicyForContext`** centralizes checks for **model allowlists**, **logger allowlists**, **MCP servers** referenced by Context **mounts** vs **`tools.servers`** / **`security.tools.allowServers`**, and **`security.models.allowProviders`** (including **`defaultProviders`**).
- **`ecp validate`:** **`--config` / `-c`**, **`--model` / `-m`**, **`--provider` / `-p`**, **`--logger` / `-l`** mirror **`ecp run`** for host policy validation before execution.
- **`ecp config add|update`:** **`--default-model`** without **`--allowed-models`** sets **`allowedModels`** to include that default; new **tools** / **loggers** / **endpoints** wiring appends the id to the matching **`security.*` allow** list (and ensures **`security.<area>`** stubs exist).
...
- **`ecp run` / `ecp validate`:** Enforce **effective model** against **`models.providers.<id>`** allowlists (implicit default included), and **fail** when a Context **mount** references an MCP server missing from **`tools.servers`** or blocked by **`security.tools.allowServers`** (instead of only logging at runtime).
