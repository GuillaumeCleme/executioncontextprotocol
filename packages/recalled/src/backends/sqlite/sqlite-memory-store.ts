 ...
 * first use and saves on close (and after each write).
 *
 * @category Backends
 */
...
export interface SqliteMemoryStoreConfig {
  /** Directory path for the SQLite file. */
  dataDir?: string;

  /** Base name for the database file. */
  filename?: string;

  /** Namespace used to separate database files. */
  namespace?: string;
}

/**
 * Create and initialize a SQLite-backed long-term memory store.
 *
 * @category Backends
 */
